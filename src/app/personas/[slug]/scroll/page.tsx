...
function buildScrollPersonaFromDB(db: Record<string, unknown>): Persona {
  ...
  const dbPersona = {
    ...
    nameZh: (decodeUnicodeEscapes(unquote(db.nameZh)) as string) || (decodeUnicodeEscapes(unquote(db.namezh)) as string) || (unquote(db.name) as string),
    nameEn: (unquote(db.nameEn) as string) || (unquote(db.nameen) as string) || (unquote(db.name) as string),
