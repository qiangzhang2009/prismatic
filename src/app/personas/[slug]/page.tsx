...
function buildPersonaFromDB(db: Record<string, unknown>): Persona {
  ...
  const dbPersona: Persona = {
    ...
    nameZh: (decodeUnicodeEscapes(unquote(db.nameZh as string)) as string) || (unquote(db.name as string) as string),
    nameEn: (unquote(db.nameEn as string)) || (unquote(db.name as string) as string),
