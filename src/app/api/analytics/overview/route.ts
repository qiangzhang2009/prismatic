    ...
    const [totalUsersRows, activeUsersRows, paidUsersRows, metricsRows] = await Promise.all([
      ...
      sql`
        SELECT
          ...
          -- Conversations updated in the period (active conversations)
          (SELECT COUNT(*) FROM conversations WHERE "updatedAt" >= ${periodStart}) as period_conversations,
          -- DAU: distinct users who sent a message today
          ...
          -- We use conversations.updatedAt >= periodStart to match "conversations updated in period"
