...
export async function GET(
  ...
  try {
    ...
    if (dbPersona && dbPersona.finalScore > 0) {
      const breakdown = (dbPersona.scoreBreakdown ?? {}) as Partial<ScoreBreakdown>;
      const overall = Math.round(
        (breakdown.voiceFidelity ?? 0) * 0.30 +
        (breakdown.knowledgeDepth ?? 0) * 0.30 +
        (breakdown.reasoningPattern ?? 0) * 0.25 +
        (breakdown.safetyCompliance ?? 0) * 0.15
      );
      ...
      return NextResponse.json({
        ...
        dataSources: (dbPersona.corpusSources ?? []) as string[],
        mainGaps: [],
        source: 'db',
      });
    }
