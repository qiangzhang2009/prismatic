...
// PRIMARY_MODES: 显示在主网格的核心 5 种体验
// DEPRECATED_MODES: 已融合或暂时搁置，以半透明样式显示在底部
const PRIMARY_MODE_IDS: Mode[] = ['solo', 'prism', 'roundtable', 'epoch', 'mission'];
const DEPRECATED_MODE_IDS: Mode[] = ['council', 'oracle', 'fiction'];
...
const MODE_CARDS: Record<Mode, {
  ...
  solo: {
    ...
    badge: '导师',
    scenarios: ['深入学习一个思想家的思维方式', '遇到困惑需要大师指点', '系统推荐最适合你的导师'],
    accentText: '#a5b4fc',
  },
  prism: {
          ...
          >
            ...
            <div className="flex-1 bg-[#0f0f18] rounded-3xl border border-white/10 shadow-2xl overflow-hidden flex flex-col relative">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                {/* ── Primary Modes (5 core experiences) ── */}
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-xs text-white/30 font-medium tracking-widest uppercase">核心体验</span>
                    <div className="h-px flex-1 bg-white/5" />
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
                    {PRIMARY_MODE_IDS.map((modeId, index) => {
                      const mode = MODES[modeId];
                      const card = MODE_CARDS[modeId];
                      const isCurrent = currentMode === modeId;
                ...
                {/* ── Deprecated / Merged Modes ── */}
                <div className="opacity-50">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-xs text-white/20 font-medium tracking-widest uppercase">已融合或活动模式</span>
                    <div className="h-px flex-1 bg-white/5" />
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {DEPRECATED_MODE_IDS.map((modeId, index) => {
                      const mode = MODES[modeId];
                      const card = MODE_CARDS[modeId];
                      const isCurrent = currentMode === modeId;
                      ...
                      return (
                        <motion.button
                          ...
                          onClick={() => handleSelect(modeId)}
                          className={cn(
                            'relative text-left rounded-2xl p-4 overflow-hidden transition-all duration-300 cursor-pointer group border',
                            isCurrent ? 'ring-2 ring-offset-2 ring-offset-[#0f0f18] opacity-100' : 'opacity-60'
                          )}
                          style={{
                            background: `linear-gradient(145deg, ${card.gradient.from}0a, ${card.gradient.to}05)`,
                            boxShadow: 'none',
                            borderColor: `${card.gradient.from}15`,
                          }}
                        >
                          {/* Deprecated label */}
                          <div className="absolute top-2 right-2">
                            <span
                              ...
                              style={{ background: `${card.gradient.from}15`, color: card.accentText }}
                            >
                              {modeId === 'council' ? '已合并' : modeId === 'oracle' ? '已融入折射' : '活动'}
                            </span>
                          </div>
                          ...
                          <div className="relative z-10">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="text-sm font-medium text-white/60">{mode.label}</span>
                            </div>
                            <div className="text-xs text-white/30 leading-relaxed">
                              {modeId === 'council' && '顾问团的能力已整合进任务特攻队'}
                              {modeId === 'oracle' && '预言能力已融入折射视图（预测类问题自动触发）'}
                              {modeId === 'fiction' && '共创故事将作为季度特别活动推出'}
                            </div>
                          </div>
                        </motion.button>
                      );
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    />
                    <span className="text-xs text-white/30 ml-1">5种核心体验</span>
                  </div>
                </div>
              </div>
            </div>
