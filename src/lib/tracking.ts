...
export async function getTrackingOverview(days: number = 7): Promise<{
  ...
  const [dauResult, wauResult, mauResult, sessionsResult, eventsResult, personasResult, conversationsResult] = await Promise.all([
    safeQuery(sql => sql`SELECT COUNT(DISTINCT visitor_id)::int as count FROM public.page_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND created_at > NOW() - INTERVAL '1 day' AND event_type IN ('pageview', 'page_view')`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT visitor_id)::int as count FROM public.page_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND created_at > NOW() - INTERVAL '7 days' AND event_type IN ('pageview', 'page_view')`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT visitor_id)::int as count FROM public.page_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND created_at > NOW() - INTERVAL '30 days' AND event_type IN ('pageview', 'page_view')`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT session_id)::int as count FROM public.page_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND created_at > NOW() - INTERVAL '${String(days)} days'`),
    safeQuery(sql => sql`SELECT COUNT(*)::int as count FROM public.prismatic_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND created_at > NOW() - INTERVAL '${String(days)} days'`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT persona_id)::int as count FROM public.prismatic_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND event_type = 'persona_view' AND created_at > NOW() - INTERVAL '${String(days)} days'`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT session_id)::int as count FROM public.prismatic_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND event_type = 'chat_start' AND created_at > NOW() - INTERVAL '${String(days)} days'`),
  ]);

  return {
    dau: (dauResult as any[])?.[0]?.count ?? 0,
    wau: (wauResult as any[])?.[0]?.count ?? 0,
    mau: (mauResult as any[])?.[0]?.count ?? 0,
    sessions: (sessionsResult as any[])?.[0]?.count ?? 0,
    totalEvents: (eventsResult as any[])?.[0]?.count ?? 0,
    totalPersonas: (personasResult as any[])?.[0]?.count ?? 0,
...
export async function getTrackingFunnel(days: number = 30): Promise<Array<{ name: string; count: number; rate: number }>> {
  ...
  const [totalVisitors, personaViews, chatStarts, modelExpands, graphClicks] = await Promise.all([
    safeQuery(sql => sql`SELECT COUNT(DISTINCT visitor_id)::int as count FROM public.page_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND event_type IN ('pageview', 'page_view') AND created_at > NOW() - INTERVAL '${String(days)} days'`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT visitor_id)::int as count FROM public.prismatic_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND event_type = 'persona_view' AND created_at > NOW() - INTERVAL '${String(days)} days'`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT visitor_id)::int as count FROM public.prismatic_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND event_type = 'chat_start' AND created_at > NOW() - INTERVAL '${String(days)} days'`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT visitor_id)::int as count FROM public.prismatic_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND event_type = 'model_expand' AND created_at > NOW() - INTERVAL '${String(days)} days'`),
    safeQuery(sql => sql`SELECT COUNT(DISTINCT visitor_id)::int as count FROM public.prismatic_events WHERE tenant_id = ${PRISMATIC_TENANT_ID} AND event_type = 'graph_node_click' AND created_at > NOW() - INTERVAL '${String(days)} days'`),
  ]);

  const total = (totalVisitors as any[])?.[0]?.count || 1;
  const toPercent = (n: number) => total > 0 ? Math.round((n / total) * 100) : 0;

  return [
    { name: '入口页浏览', count: (totalVisitors as any[])?.[0]?.count || 0, rate: 100 },
    { name: '人物浏览', count: (personaViews as any[])?.[0]?.count || 0, rate: toPercent((personaViews as any[])?.[0]?.count || 0) },
    { name: '对话开始', count: (chatStarts as any[])?.[0]?.count || 0, rate: toPercent((chatStarts as any[])?.[0]?.count || 0) },
    { name: '思维模型展开', count: (modelExpands as any[])?.[0]?.count || 0, rate: toPercent((modelExpands as any[])?.[0]?.count || 0) },
    { name: '图谱探索', count: (graphClicks as any[])?.[0]?.count || 0, rate: toPercent((graphClicks as any[])?.[0]?.count || 0) },
  ];
}
...
export async function getTrackingDeviceStats(days: number = 30): Promise<Array<{
  ...
  const total = (rows as any[]).reduce((s: number, r: Record<string, unknown>) => s + Number(r.count), 0) || 1;
  return (rows as any[]).map((r: Record<string, unknown>) => ({
