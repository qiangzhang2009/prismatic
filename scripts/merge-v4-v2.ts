/**
 * V4 → PERSONAS.ts Merger (rebuilt)
 *
 * Strategy: Collect all PERSONAS blocks from the existing file, then rebuild
 * the file from scratch with v4 data merged in. This avoids fragile in-place
 * string surgery.
 *
 * Usage:
 *   bun run scripts/merge-v4-v2.ts            # Dry run (preview)
 *   bun run scripts/merge-v4-v2.ts --write  # Apply changes
 */
...
interface Block {
  id: string;
  keyStart: number; // index of first char of `PERSONAS[...]`
  blockStart: number; // index of `{` (start of block content)
  end: number; // index after the closing `};`
  quoteStyle: "'" | '"'; // how the original file quotes the id
}

/**
 * Find all PERSONAS[...] = { ... }; blocks using character scanning.
 * Each block must start at the beginning of a line.
 */
function findAllBlocks(content: string): Block[] {
  const blocks: Block[] = [];
  // Match PERSONAS[ at line-start (with optional leading whitespace)
  const marker = "PERSONAS[";
  let i = 0;

  while (i < content.length) {
    // Find next PERSONAS[ occurrence
    const idx = content.indexOf(marker, i);
    if (idx < 0) break;

    // Verify PERSONAS[ starts at beginning of a line (col 0 or after newline)
    // Find start of this line
    let lineStart = idx;
    while (lineStart > 0 && content[lineStart - 1] !== '\n') lineStart--;
    // If line has non-whitespace before PERSONAS[, it's a section comment, not a block
    const lineBefore = content.slice(lineStart, idx);
    if (!/^[ \t]*$/.test(lineBefore)) {
      i = idx + 1;
      continue;
    }
    ...
    // Find the closing ]
    const closeIdx = content.indexOf("]", idx + marker.length);
    if (closeIdx < 0) { i = idx + 1; continue; }

    // Extract the id between [ and ]
    let id = content.slice(idx + marker.length, closeIdx);

    // Determine quote style and strip quotes
    let quoteChar = '';
    if (id.startsWith("'")) { quoteChar = "'"; id = id.slice(1); }
    else if (id.startsWith('"')) { quoteChar = '"'; id = id.slice(1); }
    if (quoteChar) id = id.replace(new RegExp(quoteChar + '$'), '');

    if (!id) { i = idx + 1; continue; }

    // Verify it is followed by = {
    const afterClose = closeIdx + 1;
    const next = content.slice(afterClose, afterClose + 5).trim();
    if (!next.startsWith('=')) { i = idx + 1; continue; }
    const braceIdx = content.indexOf('{', afterClose);
    if (braceIdx < 0) { i = idx + 1; continue; }

    // Count braces to find matching }
    let depth = 0, endIdx = -1;
    for (let j = braceIdx; j < content.length; j++) {
      if (content[j] === '{') depth++;
      else if (content[j] === '}') {
        depth--;
    ...
    if (endIdx < 0) { i = idx + 1; continue; }
    if (content[endIdx] !== ';') { i = idx + 1; continue; }

    blocks.push({
      id,
      keyStart: idx,
      blockStart: braceIdx,
      end: endIdx + 1, // include the ;
      quoteStyle: (quoteChar || "'") as "'" | '"',
    });
    i = endIdx + 1;
  }

  console.log(`  scanner: ${blocks.length} blocks found`);
  return blocks;
}
...
async function main() {
  ...
  // Step 1: Load ALL v4 data
  console.log('Loading V4 files...');
  const files = fs.readdirSync(V4_DIR).filter(f => f.endsWith('-v4.json')).sort();
  const v4Map: Record<string, { p: any; score: number; grade: string; route: string }> = {};
  for (const file of files) {
    const id = file.replace('-v4.json', '');
    const v4 = JSON.parse(fs.readFileSync(path.join(V4_DIR, file), 'utf-8'));
    v4Map[id] = {
      p: convertV4(id, v4),
      score: v4.score?.overall ?? 0,
      grade: v4.score?.grade ?? '?',
      route: v4.meta?.route ?? 'unknown',
    };
  }

  // Step 2: Read existing personas.ts
  const original = fs.readFileSync(PERSONAS_TS, 'utf-8');
  const blocks = findAllBlocks(original);
  console.log(`  Found ${blocks.length} existing persona blocks`);

  const blockById = new Map(blocks.map(b => [b.id, b]));

  // Step 3: Determine what to replace vs append
  const replacedIds: string[] = [];
  const appendedIds: string[] = [];
  for (const [id, { p, score, grade, route }] of Object.entries(v4Map)) {
    const isNew = !blockById.has(id);
    if (isNew) appendedIds.push(id);
    else replacedIds.push(id);
    console.log(`  ${isNew ? '[NEW]' : '[REPLACE]'} ${id}: score=${score} grade=${grade} route=${route}`);
  ...
  console.log(`\nSummary: ${replacedIds.length} replace, ${appendedIds.length} append (${files.length} total)`);
  ...
  // Step 4: Build new content from scratch
  // Strategy: slice the original content into regions and reassemble with v4 data

  // Find the export marker
  const exportMarker = '// ─── Legacy Exports';
  const exportIdx = original.indexOf(exportMarker);

  // Build a map of id -> new block content
  const newBlocks: Record<string, string> = {};
  for (const id of [...replacedIds, ...appendedIds]) {
    newBlocks[id] = genBlock(id, v4Map[id].p);
  }

  // Build new content:
  // 1. Take everything BEFORE the first persona block (header, imports, etc.)
  // 2. Process all existing persona blocks in order (replace or keep)
  // 3. Preserve content between last block and export marker (e.g. RISK_SUMMARY_BY_PERSONA)
  // 4. Append NEW persona blocks before the export marker

  let result = '';

  // Copy from start to the first persona block
  const firstBlockStart = blocks.length > 0 ? blocks[0].keyStart : (exportIdx >= 0 ? exportIdx : original.length);
  result += original.slice(0, firstBlockStart);

  // Process existing blocks in order
  for (const block of blocks) {
    if (replacedIds.includes(block.id)) {
      // Replace with v4 data
      result += newBlocks[block.id];
    } else {
      // Keep original
      result += original.slice(block.keyStart, block.end);
    }
    result += '\n';
  }

  // Insert new persona blocks (those not in original) before export marker
  if (appendedIds.length > 0 && exportIdx >= 0) {
    // Preserve content between last existing block and export marker (e.g. RISK_SUMMARY_BY_PERSONA)
    const lastBlockEnd = blocks.length > 0 ? blocks[blocks.length - 1].end : firstBlockStart;
    const betweenLastAndExport = original.slice(lastBlockEnd, exportIdx);
    result += betweenLastAndExport;

    // Remove trailing newlines and append new persona blocks
    result = result.replace(/\n+$/, '');
    result += '\n\n';
    for (const id of appendedIds.sort()) {
      result += newBlocks[id] + '\n\n';
    }
    result += original.slice(exportIdx);
  } else if (exportIdx < 0) {
    // No export marker found — append new personas at end
    result = result.replace(/\n+$/, '');
    result += '\n\n';
    for (const id of appendedIds.sort()) {
      result += newBlocks[id] + '\n\n';
    }
  } else if (appendedIds.length === 0 && exportIdx >= 0) {
    // No new personas, just preserve everything from last block to export
    const lastBlockEnd = blocks.length > 0 ? blocks[blocks.length - 1].end : firstBlockStart;
    result += original.slice(lastBlockEnd, exportIdx);
    result += original.slice(exportIdx);
  }

  // Step 5: Write backup and output
  if (!fs.existsSync(BACKUP_DIR)) fs.mkdirSync(BACKUP_DIR, { recursive: true });
  const backupFile = path.join(BACKUP_DIR, `personas-pre-v4-${Date.now()}.ts`);
  fs.writeFileSync(backupFile, original);
  fs.writeFileSync(PERSONAS_TS, result);

  console.log(`\nBackup: ${backupFile}`);
  console.log(`Written: ${PERSONAS_TS}`);
  console.log(`  Replaced: ${replacedIds.length} | Appended: ${appendedIds.length}`);
}
