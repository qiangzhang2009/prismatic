/**
 * Fast V4 → PERSONAS.ts Merger
 * Scans personas.ts exactly once to find all block boundaries,
 * then does targeted string replacements.
 *
 * Usage:
 *   bun run scripts/merge-v4-fast.ts            # Dry run (preview)
 *   bun run scripts/merge-v4-fast.ts --write   # Apply changes
 */
...
// ─── Single-pass block boundary scanner ──────────────────────────────────────

interface Block {
  id: string;
  start: number; // index of first char of `PERSONAS['`
  end: number;   // index after the closing `};`
}

/**
 * Scan content using regex to find line-start PERSONAS[...] = { blocks,
 * then count braces to find the matching }.
 * Handles nested braces correctly.
 */
function findAllBlocks(content: string): Block[] {
  const blocks: Block[] = [];
  // Match PERSONAS['id'] = { with optional leading whitespace (blank lines/comments before section headers are common)
  ...
  while ((match = blockRE.exec(content)) !== null) {
    ...
    if (endIdx < 0) {
      console.log(`  WARN: Could not find block end for ${id}`);
      continue;
    }

    blocks.push({ id, start, end: endIdx });
  }
...
async function main() {
  ...
  console.log('Scanning V4 files...');
  const files = fs.readdirSync(V4_DIR).filter(f => f.endsWith('-v4.json')).sort();

  // Step 1: Load and convert ALL v4 files (no I/O inside loop for file ops)
  const v4Data: Record<string, { p: any; score: number; grade: string; route: string }> = {};
  ...
  // Step 3: Find block boundaries ONCE
  const blocks = findAllBlocks(original);

  // Step 4: Determine replacements vs new appends
  const existingIds = new Set(blocks.map(b => b.id));
  const toReplace: { block: Block; ts: string }[] = [];
  const toAppend: { id: string; ts: string }[] = [];

  for (const [id, { p, score, grade, route }] of Object.entries(v4Data)) {
    const tsBlock = genBlock(id, p);
    const existing = blocks.find(b => b.id === id);
    if (existing) {
      toReplace.push({ block: existing, ts: tsBlock });
    } else {
      toAppend.push({ id, ts: tsBlock });
    }
    console.log(`  ${existing ? '[REPLACE]' : '[NEW]'}   ${id}: score=${score} grade=${grade} route=${route}`);
  ...
  console.log(`\nSummary: ${toReplace.length} to replace, ${toAppend.length} to append (${files.length} total v4 files)`);
  ...
  // Step 5: Apply replacements in reverse order (bottom-up) to preserve earlier indices
  let content = original;
  const sorted = toReplace.sort((a, b) => b.block.start - a.block.start);
  for (const { block, ts } of sorted) {
    // Try to find the block at its recorded position using both quote styles
    const keyAt = content.indexOf(`PERSONAS['${block.id}']`, block.start);
    const keyDouble = content.indexOf(`PERSONAS["${block.id}"]`, block.start);
    const keyAtValid = keyAt >= 0 && keyAt === block.start;
    const keyDoubleValid = keyDouble >= 0 && keyDouble === block.start;

    if (keyAtValid) {
      content = content.slice(0, block.start) + ts + content.slice(block.end);
    } else if (keyDoubleValid) {
      // Original used double quotes — find the block end with the double-quoted key
      const doubleEnd = content.indexOf(`\n};`, block.start + 1);
      if (doubleEnd >= 0) {
        content = content.slice(0, block.start) + ts + content.slice(doubleEnd + 3);
      } else {
        console.error(`  WARN: Could not find end for ${block.id} (double-quote variant)`);
      }
    } else {
      // Block may have shifted — search globally
      const newStart = content.indexOf(`PERSONAS['${block.id}']`);
      const newStartD = content.indexOf(`PERSONAS["${block.id}"]`);
      const resolved = newStart >= 0 ? newStart : (newStartD >= 0 ? newStartD : -1);
      if (resolved >= 0) {
        console.log(`  INFO: ${block.id} shifted from ${block.start} to ${resolved}`);
        const segEnd = content.indexOf(`\n};`, resolved + 1);
        if (segEnd >= 0) {
          content = content.slice(0, resolved) + ts + content.slice(segEnd + 3);
        }
      } else {
        console.error(`  WARN: Could not find block for ${block.id}, skipping`);
      }
    }
  }

  // Step 6: Append new personas before the export marker
  if (toAppend.length > 0 && exportIdx >= 0) {
    const before = content.slice(0, exportIdx).replace(/\n+$/, '');
    const after = content.slice(exportIdx);
    content = before + '\n\n' + toAppend.map(a => a.ts).join('\n\n') + '\n\n' + after;
  }

  // Step 7: Write backup + new file
  ...
  console.log(`  Replaced: ${toReplace.length} | Appended: ${toAppend.length}`);
}
